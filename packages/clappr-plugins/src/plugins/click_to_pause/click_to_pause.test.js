...
describe('clickToPause', function () {
  ...
  it('call pause when playing and dvr is enabled', function () { return new Promise(done => {
  ...
  it('call play when not playing and dvr is enabled', function () { return new Promise(done => {
  ...
  it('not call play nor pause when playback type is live and dvr is disable', function () { return new Promise(done => {
  ...
  it('not show cursor pointer when playback is live and drv is disable', function () { return new Promise(done => {
    vi.spyOn(ctx.container, 'getPlaybackType').mockImplementation(() => Playback.LIVE)
    vi.spyOn(ctx.container, 'isDvrEnabled').mockImplementation(() => false)
  ...
  it('show cursor pointer when playback is live and drv is enable', function () { return new Promise(done => {
    vi.spyOn(ctx.container, 'getPlaybackType').mockImplementation(() => Playback.LIVE)
    vi.spyOn(ctx.container, 'isDvrEnabled').mockImplementation(() => true)
  ...
  describe('show cursor pointer when playback is not live and', function () {
    it('playback is VOD', function () { return new Promise(done => {
    ...
    it('playback is AOD', function () { return new Promise(done => {
    ...
    it('playback is NO_OP', function () { return new Promise(done => {
  ...
  describe('on playback live and dvr enabled', function () {
    beforeEach(function () { return new Promise(done => {
      vi.spyOn(ctx.container, 'getPlaybackType').mockImplementation(() => Playback.LIVE)
      vi.spyOn(ctx.container, 'isDvrEnabled').mockImplementation(() => true)
    ...
    it('do not toggle when state do not changes', function () { return new Promise(done => {
      ctx.container.on(Events.CONTAINER_SETTINGSUPDATE, () => {
  ...
  it('call container play with parameters when received from config', function () { return new Promise(done => {
    ctx.container = new Container({
      playback: ctx.playback,
      clickToPauseConfig: { onClickPayload: { testing: true } }
    })
    const plugin = new ClickToPause(ctx.container) // eslint-disable-line
  ...
  it('call container pause with parameters when received from config', function () { return new Promise(done => {
    ctx.container = new Container({
      playback: ctx.playback,
      clickToPauseConfig: { onClickPayload: { testing: true } }
    })
