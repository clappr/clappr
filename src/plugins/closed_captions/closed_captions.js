...
export default class ClosedCaptions extends UICorePlugin {
  ...
  constructor(core) {
    ...
    this._ariaLabel =
      config && config.ariaLabel ? config.ariaLabel : 'cc-button'
    this._labelCb =
      config &&
      config.labelCallback &&
      typeof config.labelCallback === 'function'
        ? config.labelCallback
        : track => {
            return track.name
          }
  ...
  contextMenuElement(id) {
    return this.$el
      .find('ul a' + (!isNaN(id) ? '[data-cc-select="' + id + '"]' : ''))
      .parent()
  }
  ...
  render() {
    ...
    const $fullscreen = this.core.mediaControl.$el.find(
