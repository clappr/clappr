...
export class Config {
  ...
  static restore(key) {
    if (Browser.hasLocalstorage && localStorage[this._createKeyspace(key)]) {
      return this._defaultConfig()[key].parse(
        localStorage[this._createKeyspace(key)]
      )
    }
...
export function seekStringToSeconds(paramName = 't') {
  let seconds = 0
  const seekString =
    QueryString.params[paramName] || QueryString.hashParams[paramName] || ''
...
export function listContainsIgnoreCase(item, items) {
  ...
  return (
    items.find(itemEach => item.toLowerCase() === itemEach.toLowerCase()) !==
    undefined
  )
}
...
export function canAutoPlayMedia(cb, options) {
  ...
  let timeoutId = setTimeout(() => {
    setResult(
      false,
      new Error(`Timeout ${options.timeout} ms has been reached`)
    )
  }, options.timeout)
  ...
  if (promise !== undefined)
    promise.then(() => setResult(true)).catch(err => setResult(false, err))
  else setResult(true)
}
...
export class DomRecycler {
  ...
  static garbage($el) {
    ...
    if (!this.options.recycleVideo || $el[0].tagName.toUpperCase() !== 'VIDEO')
      return
    $el.children().remove()
    videoStack.push($el)
  }
