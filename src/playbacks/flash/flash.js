...
export default class Flash extends BaseFlashPlayback {
  ...
  get swfPath() {
    return template(flashSwf)({ baseUrl: this._baseUrl })
  }
  ...
  _bootstrap() {
    if (this.el.playerPlay) {
      ...
      if (this.getDuration() > 0) this._metadataLoaded()
      else
  ...
  _checkState() {
    if (this._isIdle || this._currentState === 'PAUSED') {
      ...
      this.el.getState() === 'PLAYING_BUFFERING'
    ) {
      ...
      this.trigger(
        Events.PLAYBACK_TIMEUPDATE,
        { current: 0, total: this.el.getDuration() },
        this.name
      )
  ...
  seek(time) {
    if (this.isReady && this.el.playerSeek) {
      ...
      this.trigger(
        Events.PLAYBACK_TIMEUPDATE,
        { current: time, total: this.el.getDuration() },
        this.name
      )
...
Flash.canPlay = function(resource) {
  if (!Browser.hasFlash || !resource || resource.constructor !== String) {
    ...
    return (
      ...
      !Browser.isMobile &&
      resourceParts[1].toLowerCase().match(/^(mp4|mov|f4v|3gpp|3gp)$/)
    )
  }
}
