...
describe('ContainerFactory', function() {
  ...
  it('allows overriding options', function() {
    expect(this.containerFactory.options.source).to.be.equal(
      this.options.source
    )
    expect(this.containerFactory.options.autoPlay).to.be.equal(
      this.options.autoPlay
    )
    const newSource = 'http://some.url/for/video.m3u8'
    this.containerFactory.options = $.extend({}, this.options, {
      source: newSource,
    })
