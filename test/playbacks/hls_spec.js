...
describe('HLS playback', function() {
  beforeEach(() => {
    this.isSupportedStub = sinon
      .stub(HLSJS, 'isSupported')
      .callsFake(() => true)
  })
  ...
  it('should be able to identify it can play resources independently of the file extension case', function() {
    ...
    expect(HLS.canPlay('/relative/VIDEO.M3U8')).to.be.true
    expect(HLS.canPlay('/relative/video.m3u8?foobarQuery=1234#somefragment')).to
      .be.true
    expect(
      HLS.canPlay(
  ...
  it('can play regardless of any mime type letter case', function() {
    expect(HLS.canPlay('/path/list.m3u8', 'APPLICATION/VND.APPLE.MPEGURL')).to
      .be.true
    expect(
      HLS.canPlay(
        'whatever_no_extension?foobarQuery=1234#somefragment',
