...
describe('HLS playback', function() {
  ...
  it('should be able to identify it can play resources independently of the file extension case', function() {
    expect(HlsjsPlayback.canPlay('/relative/video.m3u8')).to.be.true
    expect(HlsjsPlayback.canPlay('/relative/VIDEO.M3U8')).to.be.true
    expect(HlsjsPlayback.canPlay('/relative/video.m3u8?foobarQuery=1234#somefragment')).to.be.true
    expect(HlsjsPlayback.canPlay('whatever_no_extension?foobarQuery=1234#somefragment', 'application/x-mpegURL' )).to.be.true
    expect(HlsjsPlayback.canPlay('//whatever_no_extension?foobarQuery=1234#somefragment', 'application/x-mpegURL' )).to.be.true
  })

  it('can play regardless of any mime type letter case', function() {
    expect(HlsjsPlayback.canPlay('/path/list.m3u8', 'APPLICATION/VND.APPLE.MPEGURL' )).to.be.true
    expect(HlsjsPlayback.canPlay('whatever_no_extension?foobarQuery=1234#somefragment', 'application/x-mpegurl' )).to.be.true
  })
